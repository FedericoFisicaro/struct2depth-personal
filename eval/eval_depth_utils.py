...
def read_file_data(dataset, files, data_root):
    gt_files = []
    gt_calib = []
    im_sizes = []
    im_files = []
    ...
    #         camera_id = filename[-1]   # 2 is left, 3 is right
            ...
            if os.path.isfile(os.path.join(data_root, im)):
                gt_files.append(data_root + os.path.dirname(im) + "/left_depth" + im_id +".png")
                gt_calib.append(data_root + os.path.dirname(os.path.dirname(im)) )
                im_sizes.append(cv2.imread(data_root + im).shape[:2])
                im_files.append(data_root + im)
            else:
                num_probs += 1
